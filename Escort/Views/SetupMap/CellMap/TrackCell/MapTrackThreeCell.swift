...
class MapTrackThreeCell: UITableViewCell, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
//        cv.translatesAutoresizingMaskIntoConstraints = false
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if photoALLPlomba.count == 3 {
            photoALLPlomba.removeFirst()
        }
        return photoALLPlomba.count
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "MapCollectionPlombaCell", for: indexPath) as! MapCollectionPlombaCell
        cell.layer.cornerRadius = 5
        cell.imageAdd.image = photoALLPlomba[indexPath.row]
        if photoALLPlomba[indexPath.row] == UIImage(named: "Group 29-2") || photoALLPlomba[indexPath.row] == UIImage(named: "Group 29-2N"){
            cell.imageDelete.isHidden = true
        } else {
            cell.imageDelete.isHidden = false
        }
        cell.imageDelete.addTapGesture {
            photoALLPlomba.remove(at: indexPath.row)
            if photoALLPlomba.first != UIImage(named: "Group 29-2") && photoALLPlomba.first != UIImage(named: "Group 29-2N") {
                UserDefaults.standard.removeObject(forKey: "photoALLTrack_\(indexPath.row + 1)")
                photoALLPlomba.insert(isNight ? #imageLiteral(resourceName: "Group 29-2N") : #imageLiteral(resourceName: "Group 29-2"), at: 0)
            } else {
        ...
        cell.addTapGesture {
            if indexPath.row == 0 {
                if (photoALLPlomba.first == UIImage(named: "Group 29-2") || photoALLPlomba.first == UIImage(named: "Group 29-2N")) && photoALLPlomba.count < 3 {
                    ...
                    picker.didFinishPicking { [unowned picker] items, _ in
                        if let photo = items.singlePhoto {
                            ...
                            if let imageData = photo.image.jpeg(.lowest) {
                                photoALLPlomba.append(UIImage(data: imageData)!)
                                UserDefaults.standard.set(imageData, forKey: "photoALLPlomba_\(photoALLPlomba.count-1)")
                            }
                        }
                    ...
                    if photoALLPlomba.count == 2 {
                        self.slideshow.setImageInputs([
                            ImageSource(image: photoALLPlomba[0]),
                            ImageSource(image: photoALLPlomba[1])
                        ])
                    }
...
//        let cellWidth = (collectionView.bounds.size.width - 20) / 4
    ...
    func registerCell() {
        collectionView.register(MapCollectionPlombaCell.self, forCellWithReuseIdentifier: "MapCollectionPlombaCell")
...
//        collectionView.register(MapColectionCell.self, forCellWithReuseIdentifier: "MapColectionCell")
    ...
    func initialize() {
        ...
        setupMapTableTextField.attributedPlaceholder = NSAttributedString(string: "452460742", attributes: [NSAttributedString.Key.foregroundColor: isNight ? UIColor(red: 0.333, green: 0.333, blue: 0.333, alpha: 1.0) : UIColor(red: 0.777, green: 0.777, blue: 0.777, alpha: 1.0)])
...
//        self.levelLabel.textColor = .white
    ...
    @objc func textFieldDidChangeFirst(_ textField: UITextField) {
        ...
        plombaTrackText = textField.text!
        UserDefaults.standard.set(plombaTrackText, forKey: "plombaTrackTextMap")
