...
class SettingsReloadCell: UITableViewCell {
    
    var content: UIView!
    var setButton: UIButton!
    var sinfButton: UIButton!
    ...
    func initialize() {
        ...
        let sinfButton = UIButton()
        sinfButton.setTitle("Synchronize time".localized(code), for: .normal)
        sinfButton.setTitleColor(.lightGray, for: .highlighted)
        sinfButton.setTitleColor(.white, for: .normal)
        sinfButton.layer.cornerRadius = 10
        sinfButton.backgroundColor = UIColor(rgb: 0xE80000)
        sinfButton.titleLabel?.font = UIFont(name:"FuturaPT-Medium", size: screenWidth / 23)!
        sinfButton.translatesAutoresizingMaskIntoConstraints = false
        sinfButton.addTarget(self, action: #selector(actionSinh), for: .touchUpInside)
        self.content.addSubview(sinfButton)
        self.sinfButton = sinfButton
        ...
        NSLayoutConstraint.activate([
            ...
            self.content!.bottomAnchor.constraint(equalTo: self.sinfButton!.bottomAnchor, constant: 25),
            ...
            self.sinfButton!.leadingAnchor.constraint(equalTo: self.content!.leadingAnchor, constant: 20),
            self.sinfButton!.trailingAnchor.constraint(equalTo: self.content!.trailingAnchor, constant: -20),
            self.sinfButton!.topAnchor.constraint(equalTo: self.setButton.bottomAnchor, constant: 10),
            self.sinfButton!.centerXAnchor.constraint(equalTo: self.content.centerXAnchor),
            self.sinfButton!.heightAnchor.constraint(equalToConstant: 45),
            self.sinfButton!.bottomAnchor.constraint(equalTo: contentView.bottomAnchor, constant: -25)
