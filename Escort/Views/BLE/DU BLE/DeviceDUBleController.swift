...
class DeviceDUBleController: UIViewController {
    ...
    var delegate: ConnectedDelegate?
    var refreshControl = UIRefreshControl()
    var timer = Timer()
    let generator = UIImpactFeedbackGenerator(style: .light)
    let deviceDUSettings = DeviceDUSettingsController()
...
//        viewShow()
        NSLayoutConstraint.activate([
            ...
            self.tableView.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -90),
...
//        view.addSubview(backView)
    ...
    func createFooter() {
        ...
        cellSetting.addTapGesture { [self] in
            deviceDUSettings.delegate = self
            deviceDUSettings.delegateAlert = self
            deviceDUSettings.viewModel = viewModel
            self.navigationController?.pushViewController(deviceDUSettings, animated: true)
        ...
        cellHelp.addTapGesture { [self] in
            ...
            deviceTLVC.labelMain = "Additional Features"
//            deviceTLVC.passwortIsEnter = false
    ...
    fileprivate func registerCell() {
        ...
        tableView.register(DUSetCell.self, forCellReuseIdentifier: "DUSetCell")
    }
    ...
    fileprivate lazy var sensorImage: UIImageView = {
        let img = UIImageView(image: UIImage(named: "Слой 2-10")!)
        img.frame.origin = CGPoint(x: screenWidth/2, y: screenHeight / 4 - (iphone5s ? 70 : 70))
    ...
    let footer = UIView(frame: CGRect(x: 0, y: screenHeight - 180, width: screenWidth, height: 180))
    ...
    override func viewWillAppear(_ animated: Bool) {
        navigationCusmotizing(nav: navigationController!, navItem: navigationItem, title: "DU BLE \(nameDevice)")
        sensorImage.image = UIImage(named: "Слой 2-10")
        cellHelpIcon.image = UIImage(named: isNight ? "settings-add" : "settings-add-black")
        ...
        cellSettingName.text = "Additional Features".localized(code)
        cellSettingAddName.text = "Settings".localized(code)
...
extension DeviceDUBleController: UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate {
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        navigationItem.title = "DU BLE \(nameDevice)".localized(code)
        if indexPath.section == 0 {
            ...
            cell.label.text = "\(viewModel.duParametrs[indexPath.section])".localized(code)
            cell.labelPassword.text = level + " °"
            cell.layer.backgroundColor = UIColor.clear.cgColor
            cell.backgroundColor = .clear
            return cell
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "NumberDeviceCell", for: indexPath) as! NumberDeviceCell
            cell.label.text = "\(viewModel.duParametrs[indexPath.section])".localized(code)
            cell.labelPassword.text = modeLabel
            cell.layer.backgroundColor = UIColor.clear.cgColor
            cell.backgroundColor = .clear
            return cell
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "DUSetCell", for: indexPath) as! DUSetCell
            cell.setButton.setTitle("\(viewModel.duParametrs[indexPath.section])".localized(code), for: .normal)
            cell.layer.backgroundColor = UIColor.clear.cgColor
            cell.backgroundColor = .clear
            return cell
        }  else {
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        return viewModel.isTL ? viewModel.duParametrs.count : viewModel.thParametrs.count
    }
...
extension DeviceDUBleController: PasswordDelegate, BlackBoxTHDelegate, DUViewDelegate {
