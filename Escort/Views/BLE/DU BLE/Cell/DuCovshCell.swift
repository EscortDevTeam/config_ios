...
class DuCovshCell: UITableViewCell {
    ...
    func initialize() {
        ...
        labelMac.text = "Delta".localized(code)
        ...
        passwordSecondLabel.text = "Turn ON delay".localized(code)
        ...
        let passwordThreedLabel = UILabel()
        passwordThreedLabel.font = UIFont(name:"FuturaPT-Light", size: screenWidth / 20)
        passwordThreedLabel.textAlignment = .center
        passwordThreedLabel.text = "Turn OFF delay".localized(code)
        passwordThreedLabel.textColor = UIColor(rgb: 0x998F99)
        passwordThreedLabel.translatesAutoresizingMaskIntoConstraints = false
        passwordThreedLabel.numberOfLines = 0
        self.content.addSubview(passwordThreedLabel)
        self.passwordThreedLabel = passwordThreedLabel
        ...
        let passwordThreedTextField = TextFieldWithPadding(placeholder: "80")
        passwordThreedTextField.layer.borderColor = UIColor(rgb: 0xE7E7E7).cgColor
        passwordThreedTextField.layer.borderWidth = 1.0
        passwordThreedTextField.layer.cornerRadius = 10
        passwordThreedTextField.tintColor = .red
        passwordThreedTextField.addTarget(self, action: #selector(self.textFieldDidMax(_:)),for: UIControl.Event.editingChanged)
        passwordThreedTextField.font = UIFont(name:"FuturaPT-Light", size: screenWidth / 20)
        passwordThreedTextField.textAlignment = .left
        passwordThreedTextField.keyboardType = .numberPad
        passwordThreedTextField.textColor = UIColor(rgb: 0x998F99)
        passwordThreedTextField.translatesAutoresizingMaskIntoConstraints = false
        self.content.addSubview(passwordThreedTextField)
        self.passwordThreedTextField = passwordThreedTextField
        ...
        NSLayoutConstraint.activate([
            ...
            self.passwordThreedTextField!.trailingAnchor.constraint(equalTo: self.content!.trailingAnchor, constant: -20),
            self.passwordThreedTextField!.topAnchor.constraint(equalTo: self.passwordSecondTextField.bottomAnchor, constant: 15),
            self.passwordThreedTextField!.heightAnchor.constraint(equalToConstant: 50),
            self.passwordThreedTextField!.widthAnchor.constraint(equalToConstant: screenWidth
             / 2),

            self.passwordThreedLabel!.leadingAnchor.constraint(equalTo: self.content!.leadingAnchor, constant: 20),
            self.passwordThreedLabel!.centerYAnchor.constraint(equalTo: self.passwordThreedTextField.centerYAnchor),
            self.passwordThreedLabel!.trailingAnchor.constraint(equalTo: self.passwordThreedTextField!.leadingAnchor, constant: -20),
            ...
            self.setButton!.topAnchor.constraint(equalTo: self.passwordThreedTextField.bottomAnchor, constant: 20),
    ...
    @objc func actionSet() {
        if Int(self.passwordFirstTextField.text ?? "0") ?? 0 < 1 || Int(self.passwordFirstTextField.text ?? "0") ?? 0 > 179 {
            ...
            delegate?.buttonSetParameters(load: 23)
        }
    }
    ...
    func setupTheme() {
        if #available(iOS 13.0, *) {
            ...
            passwordThreedLabel.theme.textColor = themed{ $0.navigationTintColor }
            passwordThreedTextField.theme.textColor = themed{ $0.navigationTintColor }
            ...
            passwordThreedLabel.textColor = UIColor(rgb: isNight ? 0xFFFFFF : 0x1F1F1F)
            passwordThreedTextField.textColor = UIColor(rgb: isNight ? 0xFFFFFF : 0x1F1F1F)
        }
    }
...
extension  DuCovshCell: UITextFieldDelegate {
    func textFieldDelegate() {
        passwordFirstTextField.delegate = self
        passwordSecondTextField.delegate = self
        passwordThreedTextField.delegate = self
